...
def get_train_transforms():
    ...
    transforms = [
        ...
        mtf.SpatialPadd(keys, (128,128,128)),
        mtf.RandSpatialCropd(keys, roi_size=(128,128,128)),
...
def get_val_transforms():
    ...
    transforms = [
        ...
        mtf.EnsureChannelFirstd(keys),
        mtf.SpatialPadd(keys, (128,128,128)),
        mtf.SpatialCropd(keys, roi_center=(-64,64,64), roi_size=(128,128,128)),
        mtf.ScaleIntensityRanged(keys[:1], a_min=0, a_max=2800, b_min=0, b_max=1, clip=True),
...
class anatomixmodule(basemodule):
    def __init__(self, batch_size, split, dataset_size=1000) -> None:
        ...
        val_list = [root/(i.replace('amos', 'volume')) for i in val_list]

        train_dataroot = r'./amos_final{}'.format(len(train_list))
    ...
    def __init__(self, batch_size, split,  dataset_size=1000):
        ...
        for _ in range(dataset_size):
            ...
            self.train_items.append({
                'image': str(root/(p.replace('amos', 'volume'))),
                'label': str(root/(p.replace('amos', 'labels')))
            })
...
if __name__ == '__main__':
    ...
    dataroot = '.'
    dm = anatomixmodule(batch_size=2, split=r'./split_amos10.json')
    # dm = normalmodule(batch_size=2, split=r'./split_amos10.json', dataset_size=500)
