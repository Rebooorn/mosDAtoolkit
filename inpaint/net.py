...
def pad_x_to_y_3d(x, y):
    # [b, c, z, y, x]
    x_left = (y.size()[4] - x.size()[4]) // 2
    x_right = y.size()[4] - x.size()[4] - x_left
    y_left = (y.size()[3] - x.size()[3]) // 2
    y_right = y.size()[3] - x.size()[3] - y_left
    z_left = (y.size()[2] - x.size()[2]) // 2
    z_right = y.size()[2] - x.size()[2] - z_left
    
    x = torch.nn.functional.pad(x, [x_left, x_right, y_left, y_right, z_left, z_right,])
    return x
...
class PConvUNet(nn.Module):
    ...
    def forward(self, input, input_mask):
        ...
        for i in range(self.layer_size, 0, -1):
            ...
            h = pad_x_to_y_3d(h, h_dict[enc_h_key])
            h_mask = pad_x_to_y_3d(h_mask, h_mask_dict[enc_h_key])
...
if __name__ == '__main__':
    size = (10, 1, 141, 128, 128)
    input = torch.randn(size)
    input_mask = torch.ones(size)
    input_mask[:, :, 2:, ...][:, :, :, 2:,...] = 0

    # conv = PartialConv(1, 1, 3, 1, 1)
    # l1 = nn.L1Loss()
    # input.requires_grad = True

    # output, output_mask = conv(input, input_mask)
    # loss = l1(output, torch.randn(1, 1, 10, 10, 10))
    # loss.backward()
